...
struct {
    ...
    { "Dec 2", true, CY, 11, 2, 0, 0, 0, },
    { "2 Dec", true, CY, 11, 2, 0, 0, 0, },
    { "2 Dec 1:23", true, CY, 11, 2, 1, 23, 0, },
    { "2 Dec 1:23 PM", true, CY, 11, 2, 13, 23, 0, },
    { "12/12", true, CY, 11, 12, 0, 0, 0, },
    { "11/12", false, 0, 0, 0, 0, 0, 0, },
    { "16/12", true, CY, 11, 16, 0, 0, 0, },
    { "16/12 01:23", true, CY, 11, 16, 1, 23, 0, },
    { "16/12 01:23 PM", true, CY, 11, 16, 13, 23, 0, },
    { "12/16", true, CY, 11, 16, 0, 0, 0, },
    { "12/16 01:23", true, CY, 11, 16, 1, 23, 0, },
    { "12/16 01:23 PM", true, CY, 11, 16, 13, 23, 0, },
    ...
    { "01/01", true, CY, 0, 1, 0, 0, 0, },
    { "01/01/01", true, 1, 0, 1, 0, 0, 0, },
    { "Jan 08", true, CY, 0, 8, 0, 0, 0, },
    { "Jan 08 99", true, 99, 0, 8, 0, 0, 0, },
    { "Jan 08 1 PM", true, CY, 0, 8, 13, 0, 0, },
    ...
    for (i = 0; i < ARRAY_SIZE(tests); i++) {
        ...
        if (ret != tests[i].expected_result) {
            ...
            if (tests[i].expected_result) {
                ...
                tm = gmtime(&time);
                match =
                    tm->tm_year == (CY == tests[i].expected_year ? tm_now->tm_year : tests[i].expected_year)
                    && tm->tm_mon == tests[i].expected_mon
                    && tm->tm_mday == tests[i].expected_mday
                ...
                printf(
                    ...
                    tests[i].string,
                    tests[i].expected_mday, tests[i].expected_mon + 1, (CY == tests[i].expected_year ? tm_now->tm_year : tests[i].expected_year) + 1900, tests[i].expected_hour, tests[i].expected_min, tests[i].expected_sec,
